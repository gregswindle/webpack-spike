...
# Contributing<br>to `samplewebpack`
...
* ### 1.1. Create Issues for feature requests and defects.
    ...
    _Why:_
    > ⌦ `samplewebpack` follows an issue-driven product delivery model.
...
* ### 2.1. __Rules__
  ...
  `samplewebpack` manages contributions with the  [feature-branch-workflow](https://www.atlassian.com/git/tutorials/comparing-workflows#feature-branch-workflow).

* #### 2.1.1. Makes changes in a topic branch.

  _Why:_
  > ⌦ Use an isolated topic branch for parallel product development. Topic branches allow you to submit multiple pull requests without confusion. You can iterate without polluting the master branch with potentially unstable, unfinished code. The `samplewebpack` team uses:
  >
...
* #### 2.1.3. Branch out from `master`.
    ...
    _Why:_
    > ⌦ `samplewebpack` follows the feature-branch-workflow.
...
* ### 3.1. Use the Standard JS Style.
  ...
  `samplewebpack` follows the [Standard JS Style][standard-js-url].
...
* ### 3.2. Use ESLint to analyze source code.
  ...
  View [`samplewebpack's` ESLint rules][eslint-rules-table-url] and their enforcement.
...
[cla-url]: https://www.clahub.com/agreements/commonality/samplewebpack
...
[stackshare-url]: https://stackshare.io/commonality/samplewebpack
